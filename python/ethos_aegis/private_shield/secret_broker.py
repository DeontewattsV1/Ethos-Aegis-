...
class SecretBroker:
    ...
    Production backends should resolve ``secret://`` references through Vault,
    cloud secret managers, KMS/HSM-backed services, or equivalent infrastructure.
    """
    ...
    def register(self, secret_ref: str, material: bytes | str) -> None:
        ...
        This is an administrator/runtime operation, not an agent API.
        """
    ...
    def request_lease(
        ...
        *,
        ...
        if material is None:
            # Existence is only disclosed after the caller has passed authorization.
    ...
    def execute(
        ...
        *,
        ...
        The lease is consumed even when the adapter raises. Successful outputs are
        checked for direct secret reflection before crossing back to the caller.
        """
        ...
        if record.material in encoded_output:
            self._append_consumption_receipt(record, output_material=b"blocked-secret-reflection", success=False, now=now)
