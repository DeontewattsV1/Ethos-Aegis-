...
def canonicalize_resource_path(raw_path: str) -> str:
    ...
    The result always uses ``/`` separators. Empty paths are allowed because some
    non-filesystem capabilities operate on a resource as a whole.
    """
...
def _canonicalize(raw_value: str, *, allow_glob: bool) -> str:
    ...
    for segment in value.split("/"):
        ...
        # Windows strips trailing dots/spaces and aliases device names. Rejecting
        # them everywhere prevents a policy path from identifying different host
        # objects on Windows and POSIX systems.
        if segment.endswith((".", " ")):
